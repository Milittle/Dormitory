...
public class GoLogin extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		else if (Type.equals("维修员")) {
			if (null == new RepairDao().CheckLogin(Username, Password)) {
				Msg = "用户名或者密码错误";
				return INPUT;
			}
			else{
				//获取ID
				String Repair_ID=new RepairDao().CheckLogin(Username, Password);
				//创建session
				HttpSession session = ServletActionContext.getRequest().getSession();
				session.setAttribute("id", Repair_ID);
	...
	public static void main(String[] args) {
		System.out.println(new StudentDao().CheckLogin("0131124848", "123"));
	}
}
