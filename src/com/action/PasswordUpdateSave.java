...
public class PasswordUpdateSave extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		else if(type.equals("3"))//学生身份
		{
			...
			if (new RepairDao().CheckPassword(session.getAttribute("id").toString(), Password)) {
				RepairBean cnbean;
				cnbean=new RepairDao().GetBean(Integer.parseInt(session.getAttribute("id").toString()));
				cnbean.setRePair_Password(Password2);
	...
	public static void main(String[] args) {
		System.out.println();
		System.out.println(new RepairDao().CheckPassword("1", "123"));
	}
