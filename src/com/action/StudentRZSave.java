...
public class StudentRZSave extends ActionSupport {
	...
	private DomitoryBean domitoryBean = new DomitoryBean();

	public DomitoryBean getDomitoryBean() {
		return domitoryBean;
	}

	public void setDomitoryBean(DomitoryBean domitoryBean) {
		this.domitoryBean = domitoryBean;
	}
	...
	public String execute() throws Exception {
		...
		domitoryBean = new DomitoryDao().GetBean(Integer.parseInt(Domitory_ID));
		studentBeanList = new StudentDao().GetAllList("Student_DomitoryID=" + Domitory_ID, "");
		if (Integer.parseInt(domitoryBean.getDomitory_Number()) == studentBeanList.size()) {
			out.print("<script language='javascript'>alert('该宿舍已经满员，禁止入住操作！');history.back(-1);</script>");
			out.flush();
			out.close();
			return null;
		}
